...
import { NotificationStateCodeEnum, NotificationTypeCodeEnum } from "../lib/constants";
...
export const notificationValidationSchema = Joi.object({
    ...
    state: Joi.string().length(3).allow([NotificationStateCodeEnum.READ]),
    type: Joi.string().length(3).allow([Object.values(NotificationTypeCodeEnum)])
});
